...
class NVEmbedV2Embedder:
    ...
    def __init__(
        ...
        logger.info(f"[CapsuleBridgeRAG] loading embedding model | name={self.model_name}")
        try:
            # Used by passage splitting/token budgeting during offline indexing.
            self.tokenizer = AutoTokenizer.from_pretrained(self.model_name, trust_remote_code=True)
        except Exception as e:
            logger.warning(
                f"[CapsuleBridgeRAG] failed to load tokenizer for NV-Embed-v2; falling back to bert-base-uncased | "
