...
class OfflineIndexer:
    def __init__(self, config: CapsuleBridgeConfig, meta_dir: str) -> None:
        self.config = config
    ...
    def build_or_load(self, corpus: List[Dict[str, Any]], embedder, llm) -> Dict[str, Any]:
        if (not self.config.force_index_from_scratch) and self._index_ready():
            logger.info(f"[CapsuleBridgeRAG] offline index exists, loading | meta={self.index_meta_path}")
            with open(self.index_meta_path, "r", encoding="utf-8") as f:
                return json.load(f)
        elif (not self.config.force_index_from_scratch) and os.path.exists(self.index_meta_path):
            logger.warning(
                f"[CapsuleBridgeRAG] offline index meta exists but artifacts are missing/legacy, rebuilding | meta={self.index_meta_path}"
            )

        t0 = time.time()
        step_times: Dict[str, float] = {}
        logger.info(f"[CapsuleBridgeRAG] [Step 0] offline indexing start | docs={len(corpus)}")
        ...
        logger.info(f"[CapsuleBridgeRAG] [OFFLINE_SPLIT] docs/passages ready | docs={len(docs)} passages={len(passages)}")
        ...
        logger.info(f"[CapsuleBridgeRAG] [Step 0c] embedding docs ... | docs={len(docs)}")
        ts = time.time()
        ...
        logger.info(f"[CapsuleBridgeRAG] [Step 0c] embedding passages ... | passages={len(passages)}")
        ts = time.time()
        ...
        logger.info(f"[CapsuleBridgeRAG] [OFFLINE_INDEX] passage FAISS ready | path={self.faiss_passages_path}")

        # 3) capsule extraction (LLM, cached) per passage
        logger.info(f"[CapsuleBridgeRAG] [Step 0a] extracting capsules ... | mode={self.config.capsule_mode}")
        ts = time.time()
        ...
        logger.info(f"[CapsuleBridgeRAG] [OFFLINE_CAPSULE_EXTRACT] done | {cap_stats}")
        ...
        logger.info(f"[CapsuleBridgeRAG] [OFFLINE_ENTITY] canonicalizing entities ...")
        ts = time.time()
        ...
        logger.info(f"[CapsuleBridgeRAG] [OFFLINE_ENTITY] entities ready | entities={len(entities)}")

        # 5) embed capsules (raw) on canonical_text (or fallback to quote)
        logger.info(f"[CapsuleBridgeRAG] [Step 0c] embedding capsules ... | capsules={len(capsules)}")
        ts = time.time()
        cap_texts = []
        for c in capsules:
        ...
        logger.info(f"[CapsuleBridgeRAG] [OFFLINE_CANONICALIZE] canonicalizing capsules ...")
        ts = time.time()
        ...
        logger.info(
            f"[CapsuleBridgeRAG] [OFFLINE_CANONICALIZE] canonical capsules ready | raw={len(capsules)} canonical={len(canonical_capsules)}"
        )
        ...
        logger.info(f"[CapsuleBridgeRAG] [OFFLINE_INDEX] capsule FAISS ready | path={self.faiss_capsules_path}")
        ...
        logger.info(f"[CapsuleBridgeRAG] [Step 0b] building evidential graph ...")
        ts = time.time()
        ...
        logger.info(f"[CapsuleBridgeRAG] [Step 0] offline indexing done | meta={self.index_meta_path} total_elapsed={time.time()-t0:.2f}s")
        return index_meta
