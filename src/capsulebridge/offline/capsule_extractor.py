...
def _fallback_sentence_capsules(passage: Dict[str, Any], config: CapsuleBridgeConfig) -> List[Dict[str, Any]]:
...
def extract_capsules_for_passage(
    ...
    try:
        ...
        logger.warning(
            f"[CapsuleBridgeRAG] [OFFLINE_CAPSULE_EXTRACT] passage fallback | pid={passage.get('passage_id')} err={type(e).__name__}: {e}"
        )
