...
def canonicalize_entities(
    ...
    """
    if not bool(getattr(config, "enable_entity_canonicalization", True)):
        # Deterministic "no canonicalization" mode: each surface string becomes its own entity_id.
        # We still build entity ids because online modules (binding/jumps) expect entity_ids.
        surfaces: List[str] = []
        for cap in capsules:
            for arg in cap.get("arguments", []) or []:
                ...
                if _is_valid_entity_surface(surf):
                    surfaces.append(surf)
        for t in doc_titles or []:
            t = str(t or "").strip()
            if _is_valid_entity_surface(t):
                surfaces.append(t)

        uniq = sorted(set(surfaces), key=lambda x: (str(x).lower(), str(x)))
        entities: List[Dict[str, Any]] = []
        surface_to_eid: Dict[str, str] = {}
        for i, surf in enumerate(uniq):
            eid = f"E_{i:07d}"
            entities.append(
                {
                    "entity_id": eid,
